...
const searchCell = `                                                const isSaved = isChecked && !hasDraft && !hasError;

                                                let cellClasses = 'w-full h-full flex items-center justify-center min-h-[48px] transition-colors ';
`;

const replaceCell = `                                                if (prog.requiresRegistration === false) {
                                                    return (
...
c = c.replace(searchCell, replaceCell);

const searchFooter = `                                        return (
...
const replaceFooter = `                                        if (prog.requiresRegistration === false) {
                                            return (
...
c = c.replace(searchFooter, replaceFooter);
