...
const progFilterRegex = /const topicProgrammesInCategory = useMemo\(\(\) =>[\s\S]*?\[eligibleTopicProgrammes, topicCategory\]\);/;
...
const oldStep3BlockRegex = /\{\/\*.*?Step 3: Topic entry.*?\{\s*topicForm\.programmeId && \(\s*<motion\.div initial=\{\{ opacity: 0, y: 8 \}\} animate=\{\{ opacity: 1, y: 0 \}\} className="space-y-4">\s*<div>\s*<label className="block text-xs font-bold uppercase tracking-wider text-\[var\(--color-text-muted\)\] mb-2">\s*3.*?Enter Topic\s*<\/label>/s;

// Wait, actually I will just find the exact string "3 — Enter Topic" (ignoring dashes because they get corrupted).
// Better way: find the exact substring to replace.
let step3StartIdx = code.indexOf('Step 3: Topic entry');
if (step3StartIdx !== -1) {
    let beforeStep3 = code.lastIndexOf('{/*', step3StartIdx);
    let labelEnd = code.indexOf('</label>', beforeStep3) + 8;
    
    let oldStep3Str = code.substring(beforeStep3, labelEnd);
                  ...
                  {uniqueCands.length > 0 && !isGroup && !editTopicId && (
                    <div>
                      ...
                      >
                        ...
                        {uniqueCands.map(c => {
                            const alreadySubmitted = otherTopics[topicForm.programmeId]?.some(t => t.candidate?._id === c._id && t.team?._id === user?.team?._id || user?.team);
                            return (
    ...
    code = code.substring(0, beforeStep3) + newStep3 + code.substring(labelEnd);
}

// Close the IIFE at the end of the step 3 block
const closeRegex = /Already Submitted Topics[\s\S]*?<\/ul>\s*<\/div>\s*\)\}\s*<\/motion\.div>\s*\)\}/s;
...
console.log('Patched TeamTopicRegistrationPage.jsx using substring for step 3');
