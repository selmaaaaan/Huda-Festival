...
if (!c.includes('isRegisterModalOpen')) {
  ...
  const fetchProgrammeTopics = async (progId) => {`);
    ...
    // 3. Fix the UI Header (with perfectly balanced tags)
    const headerStart = `<div className="px-6 py-4 border-b border-[var(--color-border)] flex justify-between items-center bg-[var(--color-surface)]">`;
    const headerEnd = `</div>`;
    
    const idx1 = c.indexOf(headerStart);
    if (idx1 !== -1) {
        const idx2 = c.indexOf(headerEnd, idx1 + headerStart.length);
        const nextDiv = c.indexOf(headerEnd, idx2 + headerEnd.length); // get past the inner div
        if (nextDiv !== -1) {
             const newHeader = `<div className="px-6 py-4 border-b border-[var(--color-border)] flex justify-between items-center bg-[var(--color-surface)]">
             ...
             // Wait, it's safer to just exact replace:
        }
    }
    
    // Safer replacement for the header:
    c = c.replace(
        /<h3 className="text-base font-semibold text-\[var\(--color-text-heading\)\]">Submitted Topics<\/h3>\s*<div className="text-xs bg-\[var\(--color-surface-elevated\)\] px-3 py-1 rounded-full border border-\[var\(--color-border\)\] text-\[var\(--color-text-muted\)\]">\s*\{programmeTopics\.length\} Total\s*<\/div>/,
        `<h3 className="text-base font-semibold text-[var(--color-text-heading)]">Submitted Topics</h3>
                  <div className="flex items-center gap-3">
                    <button onClick={openRegisterModal} className="px-3 py-1 bg-[var(--color-primary)] text-white text-xs font-semibold rounded-md hover:bg-[var(--color-primary-dark)] transition-colors">
                      + Register Topic
    ...
    // 4. Edit Topic UI (from patch_edit_topic_admin)
    ...
    c = c.replace(
        /<input\s*type="text"\s*value=\{editTopicText\}\s*onChange=\{\(e\) => setEditTopicText\(e\.target\.value\)\}\s*className="flex-1 px-3 py-1.5 text-sm bg-\[var\(--color-surface\)\] border border-\[var\(--color-border\)\] rounded focus:outline-none focus:border-\[var\(--color-primary\)\]"\s*\/>/,
    ...
    // 5. Add the Modal JSX at the end
