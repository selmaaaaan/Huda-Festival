...
const regex = /\{\/\*.*?Step 3: Topic entry.*?\{topicForm\.programmeId && \(\(\) => \{.*?const reg = myRegistrations\.find.*?return \([\s\S]*?\{registeredCands\.length > 0 \? '4' : '3'\}.*?Enter Topic\s*<\/label>/s;
...
let newCode = code.replace(regex, replacementStr);
if (newCode === code) {
