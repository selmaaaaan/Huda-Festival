...
const regex = /\{topicForm\.programmeId && \(\(\) => \{\s*const reg = myRegistrations\.find.*?return \(\s*<motion\.div initial.*?className="space-y-4">\s*\{registeredCands\.length > 0 && \(\s*<div>.*?<\/div>\s*\)\}\s*<div>\s*<label.*?>.*?Enter Topic\s*<\/label>/s;

const replacementStr = `{topicForm.programmeId && (() => {
